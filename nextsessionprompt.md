...
- Pick from the open GitHub issues. Skip any that already carry the `claimed` label (check `--json number,title,labels`). Highlights:
  ...
  - #7 (CSS tokens / visual-regression coverage): its "safer first step" (app-shell grid-track + --mobile-header-height semantic tokens, plus the layout-verifier token pins) has LANDED and is pushed (commit 5febf21). The issue is still OPEN for the riskier tokenization steps (timeline/thread geometry, virtualized feed + media sizing, surface-grid card min-widths) and the desktop/wide/mobile visual + scroll-behavior verification those need. `src/styles.css` + `scripts/verify-layout-behavior.mjs` are NO LONGER off-limits; next step is more identical-value semantic tokens + a smoke. Note for verification: the current model CANNOT view CDP screenshot files - assert layout via DOM metrics instead (gridTemplateColumns, scrollWidth/clientWidth overflow, header transform) as this session did.
  ...
  - #47 (mobile-header scroll listener leak) and #36 (dead .plain-info-icon CSS) are CLOSED this batch; #7's first token step is pushed.
