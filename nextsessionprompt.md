...
- Pick from the open GitHub issues. Skip any that already carry the `claimed` label (check `--json number,title,labels`). Highlights:
  - #18/#19 (App.tsx decomposition + regex->behavioral) is the structural track. DONE so far: the MEDIA + POST-CARD + THREAD-VIEW clusters, the VirtualPostList MEASUREMENT MATH (estimateRowHeight/overscanPixelsFor/findRowIndexByOffset/computeRowOffsets/totalRowHeight/isAboveViewport -> src/lib/virtual-list.ts with a 14-case unit suite), and the VirtualPostList/MeasuredPostRow COMPONENT move (VirtualPostList + MeasuredPostRow + SCROLL_ANCHOR_* consts -> src/features/feed/VirtualPostList.tsx with a 4-case suite covering data-total-rows/data-rendered-rows, spacer windowing, onRenderedRowsChange, and the per-row error boundary; isAdultPost -> src/lib/moderation.ts). App.tsx is now ~4,297 lines; still App-local: FeedDirectoryCard, SurfaceView, SelfProfileSurface, BookmarksView, SearchView, SettingsToggle. Remaining extraction candidates: the leftover surface views, then the mega-stylesheet split (styles are still co-located in src/styles.css). Before each move, `rg "<component/classname>" scripts/verify-*.mjs` and retire any matching regex in the same commit with a behavioral test (that is the #19 pattern). Current regex tally: 18 in verify-reader-behavior.mjs + 34 in verify-layout-behavior.mjs.
