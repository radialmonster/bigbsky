...
- Pick from the open GitHub issues. Skip any that already carry the `claimed` label (check `--json number,title,labels`). Highlights:
  - #18/#19 (App.tsx decomposition + regex->behavioral) is the structural track. The MEDIA CLUSTER IS DONE: VideoEmbedCard, PostImageVideoMedia, then the full post-card cluster (PostCard, PostCardHeader, PostActionBar, PostEmbeds, MediaOnlyPostCard, QuotedPostCard + the shared Like/Bookmark/Block/Delete/Density/TagSearch contexts -> src/features/post/PostCardContexts.ts, and postPath/profilePath/handleInternalLinkClick -> src/lib/url.ts) are all extracted. Next cluster in order: the THREAD VIEW cluster - ThreadView/renderThreadNode + CombinedThreadViewCard/LongThreadCard + ThreadedPostCard + CombinedThreadSegments/CombinedThreadActionBar (still in App.tsx, ~5,685 lines). Before each move, `rg "<component/classname>" scripts/verify-*.mjs` and retire any matching regex in the same commit with a behavioral test (that is the #19 pattern). The thread cluster consumes the moved contexts via PostCardContexts.ts and the moved components via PostCard.tsx.
