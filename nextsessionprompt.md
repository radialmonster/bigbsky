...
- **Recommended next batch (in rough priority):** (a) #18 App.tsx decomposition — start with extracting a small leaf component + its CSS slice into `src/features/**` to prove the pattern (mega-stylesheet is the big risk; the pure helpers are already in src/lib with vitest suites); (b) low-risk batching with it: #19 (port a regex source-text guardrail to a behavioral test as you touch each area), #20 (dead-selector sweep), #7 (CSS tokens, claimed-check first); (c) investigate-and-decide: #10 composer GIFs/quote/video, #16 saved-feed-order cross-client, #15 report/security doc, #11/#12/#14 oEmbed/Firehose/Service-Auth writeups; (d) #24 mobile touch-viewport confirmation of the anchored scroll restore (needs a healthy deploy + signed-in CDP). As always, leave #5/#6/#22 open-ended and file a GitHub issue for anything you notice but defer.
