...
- Pick from the open GitHub issues (currently #1–#24). Skip any that already carry the `claimed` label (check `--json number,title,labels`). Highlights:
  ...
  - RECOMMENDED NEXT BATCH (in rough priority):
    (a) #18 — next the bigger presentational components that are NOT in the layout-pinned media cluster: `PostComposer`, `ImageViewer`, `DevInspector`, `ThreadView`/`ThreadEngagementPanel` (some of these trip reader regexes — e.g. DevInspector's `.dev-inspector`/`routeLabel`, PostComposer's composer logic — so check `rg "<Name|className" scripts/verify-*.mjs` first and retire reader-script regexes as you move; layout-script App.tsx regexes are NOT safe until #7 lands). Each extraction gets a co-located RTL suite.
    ...
    (c) #19 continues alongside every extraction (65 reader + 44 layout regexes left).
...
- **Recommended next batch (in rough priority):** (a) #18 — next the bigger presentational components that are NOT layout-pinned: `PostComposer`, `ImageViewer`, `DevInspector`, `ThreadView`/`ThreadEngagementPanel` (check reader regexes first and retire them as you move; the `ExploreDiscoverFeeds`/`ExploreTrendingTopics`/`ProfileFeedsTab`/`ProfileListsTab` surfaces also move eventually, but their regexes are numerous). (b) #18 item 2 — the `useRef<Record>` caches + loaders into a real cache layer (plan name-checks TanStack Query). (c) #19 continues alongside every extraction. (d) after #7 lands (claimed-check first): #20 dead-selector sweep + CSS co-location + the media-cluster extraction with layout-regex swaps. (e) investigate-and-decide: #10 composer GIFs/quote/video, #16 saved-feed-order cross-client, #15 report/security doc, #11/#12/#14 oEmbed/Firehose/Service-Auth writeups. (f) #24 mobile touch-viewport confirmation of the anchored scroll restore — needs a healthy deploy + signed-in CDP; origin still stale per #23, so don't burn the session on it. Leave #5/#6/#22 open-ended and file a GitHub issue for anything you notice but defer.
