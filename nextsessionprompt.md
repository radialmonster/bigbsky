...
- Pick from the open GitHub issues. Skip any that already carry the `claimed` label (check `--json number,title,labels`). Highlights:
  ...
  - #7 (CSS tokens / visual-regression coverage): two identical-value token batches have LANDED and are pushed - the app-shell grid-track + --mobile-header-height tokens (5febf21), and the timeline/thread geometry + surface-grid card min-widths (e911e13: --thread-indent, --timeline-padding-block/-inline/-bottom, --card-min-narrow/medium/wide/xwide). `src/styles.css` + `scripts/verify-layout-behavior.mjs` are NO LONGER off-limits. Remaining: the virtualized feed/media sizing tokenization (VirtualPostList measurement, image/video min heights, link-card thumb clamp, wide-desktop embed columns) and any riskier value CHANGES that need desktop/wide/mobile visual + scroll-behavior verification. Note for verification: the current model CANNOT view CDP screenshot files - assert layout via DOM metrics instead (gridTemplateColumns, scrollWidth/clientWidth overflow, header transform) as prior sessions did.
  - #20 (CSS dead-selector sweep): partial sweep done (removed ~290 lines of 11 dead selector groups, incl. the #36 .plain-info-icon orphan). Re-run the zero-usage cross-check AFTER the media/ThreadView extraction (#18) - those clusters may retire more classes. Watch for dynamically-built class names (e.g. `toast-${kind}` kept `toast-error`/`toast-success`).
  - #19 (regex source-text tests -> behavioral tests) continues alongside every extraction. Reader verifier is 31 requirePattern + 3 requireInfoPattern + 2 forbidPattern; layout is 38 requirePattern + 12 forbidPattern. Retire reader regexes as components/lib modules move; the layout regexes (media aspectRatio/videoFrameStyle pins) can only be swapped once #7 lands.
