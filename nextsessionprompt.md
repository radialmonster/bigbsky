...
- Current worktree state (verify again at session start): as of 2026-08-02 the tree is CLEAN and in sync with origin/main; the #19 VirtualPostList-math slice (src/App.tsx + src/lib/virtual-list.ts + scripts/verify-layout-behavior.mjs) is committed + pushed. loop.bat/loop.ps1 are the shared watchdog loop; run.log* + .loop-tmp are gitignored. Do not revert unrelated user changes.
...
- Pick from the open GitHub issues. Skip any that already carry the `claimed` label (check `--json number,title,labels`). Highlights:
  - #18/#19 (App.tsx decomposition + regex->behavioral) is the structural track. The MEDIA + POST-CARD + THREAD-VIEW clusters and the VirtualPostList MEASUREMENT MATH are DONE: VideoEmbedCard, PostImageVideoMedia, the full post-card cluster (PostCard, PostCardHeader, PostActionBar, PostEmbeds, MediaOnlyPostCard, QuotedPostCard + contexts -> src/features/post/PostCardContexts.ts, and postPath/profilePath/handleInternalLinkClick -> src/lib/url.ts), the thread-view cluster (ThreadView + renderThreadNode/renderThreadContextNode + CombinedThreadViewCard/LongThreadCard/ThreadedPostCard/CombinedThreadSegments/CombinedThreadActionBar + replyRootRefForPost/useComposerTargets -> src/features/thread/ThreadView.tsx; normalizeLinkHref/extractFacetLinks -> src/lib/url.ts), and the VirtualPostList row math (estimateRowHeight/overscanPixelsFor/findRowIndexByOffset/computeRowOffsets/totalRowHeight/isAboveViewport -> src/lib/virtual-list.ts with a 14-case unit suite; retired 4 layout regexes). App.tsx is now ~4,850 lines; the VirtualPostList COMPONENT itself (render wiring) and MeasuredPostRow/FeedDirectoryCard/SurfaceView/BookmarksView/SearchView/SettingsToggle are still App-local. Remaining extraction candidates: the mega-stylesheet split (styles are still co-located in src/styles.css), the VirtualPostList/MeasuredPostRow component move, and the leftover surface views. Before each move, `rg "<component/classname>" scripts/verify-*.mjs` and retire any matching regex in the same commit with a behavioral test (that is the #19 pattern). Current regex tally: 22 in verify-reader-behavior.mjs + 37 in verify-layout-behavior.mjs.
  - #20 (CSS dead-selector sweep) is CLOSED (2026-08-02): the zero-usage cross-check after the media/thread-view extractions is clean - zero dead selectors (only the dynamic `toast-${kind}` pair remains). If you add new CSS classes, re-check they are used; no further sweep action needed.
  - #52 (filed follow-up to #7): audit real VALUE CHANGES to media/layout geometry (mobile 55px header offsets, VirtualPostList defaultRowHeight estimates vs measured heights, media-density min heights, quote-card geometry, the 1900px two-zone card columns) - each change needs desktop/wide/mobile CDP verification (DOM metrics, not screenshots: gridTemplateColumns, scrollWidth/clientWidth overflow, computed min/max heights) plus a scroll-restore smoke. Do not change values blindly. Note: the estimate values now live in src/lib/virtual-list.ts `estimateRowHeight` (compact 112 / media 360 / comfortable 260).
