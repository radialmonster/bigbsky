...
- Current worktree state (verify again at session start): uncommitted changes exist in `src/App.tsx`, `src/api.ts`, `src/lib/loaders.ts`, `src/lib/loaders.test.ts`, both `scripts/verify-*.mjs`, `src/styles.css`, `.gitignore`; `run-prompt.bat`/`run-prompt.ps1` are deleted; `loop.bat`/`loop.ps1` are untracked. `src/styles.css` + `scripts/verify-layout-behavior.mjs` carry in-flight #7 CSS-token edits - do NOT touch or stage those two files until #7 lands. Do not revert unrelated user changes.
