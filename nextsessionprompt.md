...
- Pick from the open GitHub issues (currently #1–#24). Skip any that already carry the `claimed` label (check `--json number,title,labels`). Highlights:
  ...
  - App.tsx decomposition (#18) — components moved so far into src/features/**: feed cluster (AutoLoadMoreButton/PostRowFallback/findScrollParent/BackToTopButton), common cluster (Avatar/State/MediaGate/ToastHost), rightRail ProfileContextPanel, and post leaves (ReplyLimitedNotice, RichText renderRichText, ExternalLinkCard, UnsupportedEmbedNotice, useReplyGate; postBskyUrl -> src/lib/url). Remaining: the bigger presentational components (see batch below), the PostCard/media cluster (BLOCKED until #7 — see lesson below), the useRef<Record> cache-layer with loaders, and the mega-stylesheet split.
  - Replace regex source-text tests (#19) — reader script is at 68 assertions (retired 108–110 in the latest slice), layout script still 44. Continue retiring reader regexes as components move; the layout-script App.tsx regexes (VirtualPostList measurement + the media aspectRatio/videoFrameStyle pins) can only be swapped once #7 lands.
  ...
  - RECOMMENDED NEXT BATCH (in rough priority):
    (a) #18 — extract the remaining right-rail / small presentational components next: `PinnedProfilesPanel`, `PinnedSearchesPanel`, `RecentPanel`, `FeedContextPanel`, `TrendingPanel` (retire reader regex 69 + add test), `DiscoverFeedCard`, `HomeSourcePicker`, `ProfileDetailHeader`. Low-risk slices: check `rg "<Name|className" scripts/verify-*.mjs` first; reader-script regexes are safe to retire, layout-script App.tsx regexes are NOT until #7 lands.
    (b) #18 item 2 — the `useRef<Record>` caches + their loaders into a real cache layer (plan name-checks TanStack Query). Bigger structural slice; feasible without touching styles/verify scripts.
    (c) #19 continues alongside every extraction.
    (d) after #7 lands (claimed-check first): #20 dead-selector sweep + co-locate each extracted component's CSS slice, AND finally extract the PostCard/media cluster (PostCard/PostEmbeds/QuotedPostCard/PostImageVideoMedia/VideoEmbedCard/MediaOnlyPostCard) by swapping the layout-script media regexes for RTL tests.
    (e) investigate-and-decide: #10, #16, #15, #11/#12/#14.
...
- **The component-extraction pattern for #18 is proven and scaling.** Three slices landed: `src/features/feed/AutoLoadMoreButton.tsx` (AutoLoadMoreButton + PostRowFallback + findScrollParent), the leaf/state cluster (`Avatar`, `LoadingState`, `ErrorState`, `SensitiveMediaGate`, `MediaHiddenButton`, `ReplyLimitedNotice`) into `src/features/common/*` + `src/features/post/`, and the small presentational cluster (`BackToTopButton` → `src/features/feed/`, `ToastHost` + its `ToastKind`/`ToastMessage` types → `src/features/common/`, `ProfileContextPanel` → `src/features/rightRail/`, `RateLimitState`/`EmptyState`/`EndOfFeedCard` appended to `src/features/common/State.tsx`). Each component gets a co-located RTL suite (`src/**/*.test.tsx`, jsdom env already configured; `@testing-library/react` is a devDependency). CRITICAL gotcha: `npm run build` runs `scripts/verify-reader-behavior.mjs`/`verify-layout-behavior.mjs`, whose regexes match App.tsx source text — when you move a component out of App.tsx, check those scripts for regexes matching its moved source and delete/swap them in the same commit or the build fails. IMPORTANT caveat learned this slice: not every extraction needs a regex swap — the leaf and small presentational components had NO matching verifier regexes, so plain verbatim moves + tests suffice; check `rg "<ComponentName|className" scripts/verify-*.mjs` before assuming. Also: jsdom's a11y tree hides `alt=""` (decorative) images from `getByRole("img")` — query `container.querySelector("img.avatar")` instead. Drop now-unused imports at the extraction site (e.g. App's `scrollFeedToTop` became unused after BackToTopButton moved out). CSS co-location stays deferred (the mega-stylesheet split is the big remaining #18 risk) — `src/styles.css` is untouched until #7 lands.
...
- **Recommended next batch (in rough priority):** (a) #18 — extract the remaining right-rail/small presentational components (`PinnedProfilesPanel`, `PinnedSearchesPanel`, `RecentPanel`, `FeedContextPanel`, `TrendingPanel` [retire reader regex 69], `DiscoverFeedCard`, `HomeSourcePicker`, `ProfileDetailHeader`), each with an RTL suite; the PostCard/media cluster stays blocked until #7 (see above). (b) #18 item 2 — the `useRef<Record>` caches + loaders into a real cache layer (plan name-checks TanStack Query). (c) #19 continues alongside every extraction. (d) after #7 lands (claimed-check first): #20 dead-selector sweep + CSS co-location + the media-cluster extraction with layout-regex swaps. (e) investigate-and-decide: #10 composer GIFs/quote/video, #16 saved-feed-order cross-client, #15 report/security doc, #11/#12/#14 oEmbed/Firehose/Service-Auth writeups. (f) #24 mobile touch-viewport confirmation of the anchored scroll restore — needs a healthy deploy + signed-in CDP; origin still stale per #23, so don't burn the session on it. Leave #5/#6/#22 open-ended and file a GitHub issue for anything you notice but defer.
