...
- **The component-extraction pattern for #18 is proven and scaling.** Two slices landed: `src/features/feed/AutoLoadMoreButton.tsx` (AutoLoadMoreButton + PostRowFallback + findScrollParent) and the leaf/state cluster (`Avatar`, `LoadingState`, `ErrorState`, `SensitiveMediaGate`, `MediaHiddenButton`, `ReplyLimitedNotice`) into `src/features/common/*` + `src/features/post/`, with the pure moderation-label helpers (`moderationLabelText`/`isSensitiveLabel`/`sensitiveMediaValues`) into `src/lib/moderation.ts`. Each component gets a co-located RTL suite (`src/**/*.test.tsx`, jsdom env already configured; `@testing-library/react` is a devDependency). CRITICAL gotcha: `npm run build` runs `scripts/verify-reader-behavior.mjs`/`verify-layout-behavior.mjs`, whose regexes match App.tsx source text — when you move a component out of App.tsx, check those scripts for regexes matching its moved source and delete/swap them in the same commit or the build fails. IMPORTANT caveat learned this slice: not every extraction needs a regex swap — the leaf components had NO matching verifier regexes, so plain verbatim moves + tests suffice; check `rg "<ComponentName|className" scripts/verify-*.mjs` before assuming. Also: jsdom's a11y tree hides `alt=""` (decorative) images from `getByRole("img")` — query `container.querySelector("img.avatar")` instead. Drop now-unused lucide imports at the extraction site (e.g. `EyeOff`). CSS co-location stays deferred (the mega-stylesheet split is the big remaining #18 risk) — `src/styles.css` is untouched until #7 lands.
- **Housekeeping: #17 and #21 are CLOSED as accepted-by-design records** (reply-count math caveat and readCollapsedFeedGroups passthrough, both documented inline). Don't reopen unless their call sites change. #19 stays open (≈115 regex assertions remain across the two verify scripts; the leaf cluster added 19 behavioral tests but retired no regexes) — continue the migration as you extract each component.
- **Recommended next batch (in rough priority):** (a) #18 — extract the medium/big presentational components next. Best next candidates in increasing size: `BackToTopButton`, `ToastHost`, `RateLimitState`, `ProfileContextPanel`, then `ExternalLinkCard` and `PostCard` (these carry several verifier regexes, so budget for regex-swap work), leaving `PostComposer`/`ThreadView`/`ImageViewer`/`DevInspector` for later; (b) after #7 lands (claimed-check first): #20 dead-selector sweep + co-locate each extracted component's CSS slice out of the mega-stylesheet; (c) #19 continues alongside every extraction; (d) investigate-and-decide: #10 composer GIFs/quote/video, #16 saved-feed-order cross-client, #15 report/security doc, #11/#12/#14 oEmbed/Firehose/Service-Auth writeups; (e) #24 mobile touch-viewport confirmation of the anchored scroll restore (needs a healthy deploy + signed-in CDP; origin still stale per #23 — don't burn the session on it). Leave #5/#6/#22 open-ended and file a GitHub issue for anything you notice but defer.
