...
- Pick from the open GitHub issues (currently #1–#24). Skip any that already carry the `claimed` label (check `--json number,title,labels`). Highlights:
  ...
  - App.tsx decomposition (#18) — components moved so far into src/features/**: feed cluster (AutoLoadMoreButton/PostRowFallback/findScrollParent/BackToTopButton/HomeSourcePicker/DiscoverFeedCard), common cluster (Avatar/State/MediaGate/ToastHost/useResetTimeout), rightRail (ProfileContextPanel/RecentPanel/FeedContextPanel/TrendingPanel/PinnedSearchesPanel/PinnedProfilesPanel), post leaves (ReplyLimitedNotice, RichText renderRichText, ExternalLinkCard, UnsupportedEmbedNotice, useReplyGate; postBskyUrl -> src/lib/url), profile (ProfileDetailHeader + profileTabs/ProfileTab), post cluster (ThreadEngagementPanel, ImageViewer + ImageViewerState/ImageViewerImage types), and the FULL composer cluster (PostComposer + PostLanguagePicker + EmojiPicker + the post-language helpers/POST_LANGUAGE_OPTIONS/languageDisplayName/readReplyDefaultLanguage/ComposerImageState → src/features/composer/PostComposer.tsx; App re-imports PostComposer/POST_LANGUAGE_OPTIONS/languageDisplayName/composerDraftStorageKey/PostRefValue). Remaining: the bigger presentational components (see batch below), the PostCard/media cluster (BLOCKED until #7 — see lesson below), the useRef<Record> cache-layer with loaders, and the mega-stylesheet split.
  - Replace regex source-text tests (#19) — reader script is at 64 assertions (no composer regexes existed to retire in slice 7; the cluster had none), layout script still 44. Continue retiring reader regexes as components move; the layout-script App.tsx regexes (VirtualPostList measurement + the media aspectRatio/videoFrameStyle pins) can only be swapped once #7 lands.
  ...
  - RECOMMENDED NEXT BATCH (in rough priority):
    (a) #18 — `PostComposer` is DONE (slice 7). Next the `ThreadView`/`renderThreadNode` + `CombinedThreadViewCard`/`LongThreadCard` cluster (note: `threadUnavailableState` and the `.thread-alert` tone regexes in the reader script live in that cluster — retire them as you move it), then the `ExploreDiscoverFeeds`/`ExploreTrendingTopics`/`ProfileFeedsTab`/`ProfileListsTab` surfaces (their reader regexes are numerous — see #19). Each extraction gets a co-located RTL suite.
    ...
    (c) #19 continues alongside every extraction (64 reader + 44 layout regexes left).
...
- **Recommended next batch (in rough priority):** (a) #18 — `PostComposer` is DONE (slice 7, ~803 lines moved). Next the `ThreadView`/`renderThreadNode`+`CombinedThreadViewCard`/`LongThreadCard` cluster (note: `threadUnavailableState` and the `.thread-alert` tone regexes in the reader script live in that cluster — retire them as you move it), then the `ExploreDiscoverFeeds`/`ExploreTrendingTopics`/`ProfileFeedsTab`/`ProfileListsTab` surfaces (their reader regexes are numerous — see #19). (b) #18 item 2 — the `useRef<Record>` caches + loaders into a real cache layer (plan name-checks TanStack Query). (c) #19 continues alongside every extraction (reader script now 64 regexes). (d) after #7 lands (claimed-check first): #20 dead-selector sweep + CSS co-location + the media-cluster extraction with layout-regex swaps. (e) investigate-and-decide: #10 composer GIFs/quote/video, #16 saved-feed-order cross-client, #15 report/security doc, #11/#12/#14 oEmbed/Firehose/Service-Auth writeups. (f) #24 mobile touch-viewport confirmation of the anchored scroll restore — needs a healthy deploy + signed-in CDP; origin still stale per #23, so don't burn the session on it. Leave #5/#6/#22 open-ended and file a GitHub issue for anything you notice but defer.
