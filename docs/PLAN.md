...
- Media, GIF/video, alt text, and content-label rendering states. Status: improved; image alt badges, overflow image-count badges when more than four images are present, image-viewer alt text, normalized content-label chips, and AppView video/GIF cards now render from loaded data. Video/GIF cards use native browser controls for playable playlist URLs, keep thumbnail/open-media fallbacks, and expose alt text without BigBSky media proxying; richer cross-browser HLS playback remains deferred unless a small client library is justified.
