...
- [ ] **"Permissions updated" re-auth prompt** — on session restore, compare the session's granted scope to the desired `OAUTH_SCOPE`; if they differ, offer a one-click re-authorize. Copy says "updated", not "reduced" (scopes may grow). Needed because long-lived refresh tokens keep an old scope until re-auth — and several tasks below add scopes.
