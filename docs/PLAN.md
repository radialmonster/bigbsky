...
- [ ] **Image upload on posts** — attach images via `blob:image/*` (the composer currently has local placeholders only). (Scope granted.)
...
- [ ] **Investigate the "Project" feed group** — the static "BigBSky Route Test" placeholder feed was removed (it just pointed at the public Discover/whats-hot feed), but the `"Project"` group label is still applied to dynamically-created feed sources (discovered feeds, lists-opened-as-feeds) at App.tsx (~938/3376/3539/4527) and a "known feed" filter at ~381. Review whether "Project" is the right label/grouping for those dynamic sources or whether it's leftover dev scaffolding that should be renamed/removed. Confirm the feed selector no longer shows an empty "Project" group.
