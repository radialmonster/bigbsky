...
- Verify DOM size remains bounded after scrolling multiple timeline pages. Status: partial; measured-row virtualization is implemented for Feed/profile timelines with top/bottom spacers and rendered-row reporting, while full several-page stress testing remains pending.
...
- Media, GIF/video, alt text, and content-label rendering states. Status: improved; image alt badges, overflow image-count badges when more than four images are present, image-viewer alt text, normalized content-label chips, AppView record-with-media quote/image cards, and AppView video/GIF cards now render from loaded data. Adult/graphic media is hidden behind a click-to-reveal content warning before any thumbnail loads — for both top-level post media and quoted-post media (via a shared SensitiveMediaGate) — and a Settings "Content & Media" toggle (browser-local, hidden by default for all users) can switch to always showing labeled media. Video/GIF cards use native browser controls for playable playlist URLs, keep thumbnail/open-media fallbacks, and expose alt text without BigBSky media proxying; richer cross-browser HLS playback remains deferred unless a small client library is justified.
- Muted/blocked content handling as exposed by APIs. Status: partial; post cards now surface AppView viewer flags such as muted threads, limited replies, embedding-disabled state, and sensitive labels in a local moderation notice, post images/video carrying adult/nudity/porn/sexual/graphic/gore/violence labels (checked on both the post and the author/account labels the public AppView returns) are gated behind a click-to-reveal "Sensitive content — Show" warning (with a Hide control after reveal) that defaults to hidden for everyone and can be switched to always-show with the Settings "Content & Media" toggle (`bigbsky:show-nsfw`), and standalone thread branches render typed blocked/deleted/not-found/rate-limited/unavailable alerts from AppView thread nodes. Richer authenticated moderation states remain pending.
...
- Wider active Feed timeline formatting while preserving endless-scroll behavior. Status: improved; the reader is widened by a single fluid `1fr` content column with fixed narrow rails, so the content column always absorbs remaining width and is the widest column at every screen size without any per-screen-size grid breakpoints. Endless-scroll virtualization is unchanged. To keep the very wide column pleasing, post/quote prose holds a single-constant readable measure (`max-width: 70ch`) while cards, media, link cards, and quote embeds still use the full card width — verified live at 2560px (content column 1876px, card 1809px, media 1775px, prose 641px) and inert on narrow/mobile columns (375px viewport → prose 481px, no cap applied).
...
- Wide post-card layout variants. Status: improved; post cards are classified as text-only, media, link, or quote variants. In the very wide reader column the post's text-flow elements (repost reason, author header, label badges, action row) now align to a single readable left column (`max-width: 720px`) matching the `70ch` prose measure, so a wide card reads as one coherent unit instead of stretching the header full-width and flinging the More button to the far card edge — while media, link, and quote embeds intentionally break out to the full card width. Verified live at 2560px: on a 1809px card, header/actions = 720px, prose = 641px, More button right edge at ~735px from the card's left, and media still spans 1775px; the Compact two-zone layout is preserved (header in zone 1, media in zone 2).
...
- Contextual right rail and preview side panel. Status: improved; the right rail adapts between Feed/profile context, link previews, recent history, Feed Map, local pinned searches, and loaded-post hashtag trends, and now also a context-preserving author peek: clicking a post's avatar opens an author preview panel in the right rail (provided via `AuthorPeekContext` so it is not threaded through every card) that renders the embedded author instantly, then makes at most one `app.bsky.actor.getProfile` request to fill in bio + follower/following/post counts (reusing the in-memory profile cache when present), with Close and "Open full profile" actions. Opening it never navigates, so the active feed scroll position is preserved. Verified live at 2560px: clicking an avatar at scrollTop 400 kept the route at `/feed/discover` and scroll at 400 while the panel loaded WeRateDogs' bio and counts (538,684 followers).
...
- Endless scroll is acceptable and should remain natural.
...
- Avoid horizontal scrolling as a core interaction pattern. It should not be required for Feed selection, reading posts, accessing actions, or navigating major surfaces.
