...
- [ ] **Build & curate block lists** — UI to create a moderation list (`app.bsky.graph.list`) and add/remove accounts (`app.bsky.graph.listitem`), plus subscribe to an existing block list (`app.bsky.graph.listblock`). NOTE: subscribing to a *mute* list calls `app.bsky.graph.muteActorList`, which is NOT in the enumerated scope — add `rpc:app.bsky.graph.muteActorList?aud=…` (+ `muteActor`/`muteThread`/`unmute*` if used) and re-consent.
