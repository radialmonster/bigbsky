...
Done in this pass: minimized + enumerated OAuth scopes (no misleading "Chat" on consent), single-source scope (JSON), Chat + Edit-profile delegate to bsky.app, account Follow write, post Like write, account Block/unblock write (profile-header control, confirm-on-block, seeded from `viewer.blocking`), authenticated reads for viewer-state everywhere (author-feed, post-thread/branches, and post search now route through the signed-in agent so Like/Follow/Block initial state is correct on every surface), profile replies-only empty-state, left-rail tooltip readability, removed Feed Map and Build Posture panels.
