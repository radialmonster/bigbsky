...
- Personal feeds/lists. Note: public profiles now surface an account's published Feeds and Lists (read-only discovery). A future item is an in-app Bluesky list-timeline reader (via `app.bsky.feed.getListFeed`) so list cards can open as an in-app source like Feeds, instead of linking out to Bluesky.
...
- Search and trending topics. Status: partial; public post/profile/Feed search is implemented, current searches can be pinned locally, the Explore surface now loads live "Trending Topics" from the public `app.bsky.unspecced.getTrendingTopics` endpoint (each topic opens an in-app post search), and the right rail still derives lightweight hashtag trends from loaded posts with static fallback topics.
...
- At 2560px, the feed presentation becomes richer or more useful instead of expanding empty gutters. Status: locally guarded by `npm run build`; the very-wide breakpoint increases the active reader track and turns compact rich post cards into two-zone desktop cards instead of only expanding side gutters.
