...
- Preserve feed scroll position when opening post/thread/profile/media context. Status: improved; feed/profile scroll offsets are cached locally for shell navigation, cached feed/profile loads restore their prior offsets, feed selector switching no longer forces the timeline back to the top, and browser-local Saved/List timelines now restore route-specific scroll offsets when revisited in the SPA. Full browser reload restoration still needs verification.
