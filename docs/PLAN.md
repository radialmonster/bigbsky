...
- [ ] **Real Lists page (replace local-only lists)** — `/lists` currently only manages browser-session "local list workspaces"; that is not wanted. Surface the user's actual Bluesky lists instead: the lists they created/own (`app.bsky.graph.getLists` for the signed-in DID) and the lists they subscribe to (curation lists pinned in saved-feeds preferences; mute lists / block lists from `getPreferences`). Open a list as a timeline via `app.bsky.feed.getListFeed` (already wired for `at://…graph.list` routes). Remove or hide the local-only list workspaces. Reads (`getLists`/`getList`/`getListFeed`) are already in scope.
