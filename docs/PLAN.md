...
- [ ] **Block / unblock accounts** — add a Block control on the profile header (next to Follow), writing `app.bsky.graph.block`. `viewer.blocking` is already available via `getProfileAuthed`, so seed/toggle like Follow. (Scope granted.)
