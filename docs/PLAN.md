...
- Wider active Feed timeline formatting while preserving endless-scroll behavior. Status: improved; the reader column now keeps widening across screen tiers. At =1900px Balanced uses `minmax(980px, 1fr)`, and Wide/Focus now keep effect (Wide `minmax(1100px, 1.6fr)`, Focus `minmax(1280px, 1.8fr)`); a new =2560px ultrawide tier pushes the center track to `minmax(1200px/1500px/1700px, ...)`. Endless-scroll virtualization is unchanged. Long text-only prose is capped to a `68ch` measure on wide screens so very wide cards stay readable while media/link/quote embeds still use full card width.
...
- Configurable feed width and density. Status: improved; browser-local Balanced, Wide, and Focus width modes plus existing density modes. Fixed a widescreen bug where the `@media (min-width: 1900px)` `.app-shell` rule overrode `.width-wide`/`.width-focus` (equal specificity, later in source) so Wide/Focus had no effect on large monitors and Focus left an empty 340px gutter where the hidden right rail was; the breakpoint now has explicit `.app-shell.width-wide`/`.app-shell.width-focus` grids, and a dedicated =2560px ultrawide tier widens all three modes further.
...
- At 2560px, the feed presentation becomes richer or more useful instead of expanding empty gutters. Status: locally guarded by `npm run build`; the =1900px breakpoint increases the active reader track and turns compact rich post cards into two-zone desktop cards, and a dedicated =2560px ultrawide tier widens the reader column for all width modes (Balanced/Wide/Focus), grows reserved media height (`--post-embed-min: 280px`), and keeps rails narrow instead of spending the extra width on gutters.
