...
- [ ] **Subscribe to *mute* lists (scope add)** — block-list subscription (`app.bsky.graph.listblock`) shipped; subscribing to a list as a *mute* list still needs `app.bsky.graph.muteActorList`, which is NOT in the enumerated scope. Add `rpc:app.bsky.graph.muteActorList?aud=…%23bsky_appview` (+ `muteActor`/`muteThread`/`unmute*` if a per-account mute control is added) and re-consent. Batch with the Notifications scope adds below to minimize re-consents; the "Permissions updated" prompt will surface the re-auth.
- [ ] **Build a real Notifications view** — replies/mentions/likes/reposts/follows/quotes from the authenticated AppView. NEEDS NEW SCOPES: add `rpc:app.bsky.notification.listNotifications` / `getUnreadCount` / `updateSeen` (`?aud=…%23bsky_appview`) + re-consent, because the enumerated scope no longer includes the old `rpc:*` wildcard. (Also tracked under MVP Scope.)
