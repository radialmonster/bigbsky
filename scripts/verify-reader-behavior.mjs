...
requirePattern(/function scrollOffsetTo\(timeline: HTMLElement \| null[\s\S]*isMobileScroller\(\)[\s\S]*window\.scrollTo[\s\S]*timeline\?\.scrollTo/s, "scroll restoration should target the document on mobile and the timeline on desktop");
