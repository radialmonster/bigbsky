...
- [ ] Standardize composer controls for posts and replies.
  ...
  - Remaining (still open under this task): **GIFs** (Tenor embed flow), **emoji picker**, and **post-language selector** — none added yet, on either composer. The post `Composer` also still lacks a language selector and emoji/GIF controls. These are larger (Tenor integration, picker UI, lang metadata) and should verify bsky.app's exact API calls first. Captured as the next steps here.
  - Relevant files/functions found:
